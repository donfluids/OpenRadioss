! External-blast-on-a-rigid-cube capability smoke test (M5.3).
!
! A 1 g TNT-equivalent point energy is deposited at (0.3,0.3,0.3) in a
! 0.6 m air cube. A 0.1 m rigid cube sits at [0.4,0.5]x[0.25,0.35]^2.
! Cube faces have slip-wall BCs; outer domain faces are farfield.
! Probes sample p(t) just upstream of the cube front face and just
! downstream of the back face.
!
! Pass criteria (smoke-test scope; ConWep reflected-overpressure
! validation is a follow-up to this milestone and requires a finer mesh
! plus a full-scale charge):
!   1. Front-face peak overpressure > 30 kPa — a shock arrives and
!      reflects off the rigid wall.
!   2. Front-face peak overpressure > back-face peak overpressure —
!      the cube actually occludes / shadows the back face.
program test_blast_cube
   ...
   real(wp), parameter :: FRONT_MIN_DP_KPA = 30.0_wp

   real(wp) :: p_peak_local, p_peak_global, dp_peak_kpa
   real(wp) :: dp_front_kpa, dp_back_kpa
   real(wp) :: p_amb
   ...
   if (nargs < 1) then
      if (ctx%is_root) write(error_unit,'(A)') 'usage: test_blast_cube <input.nml>'
   ...
   if (ctx%is_root) write(*,'(A,F8.4,A,F8.4,A,1PE12.4)') &
      'blast_cube: tnt_mass=', p%tnt_mass, ' kg  blast_radius=', p%blast_radius, &
   ...
   p_amb        = p%ambient_p
   dp_front_kpa = -huge(1.0_wp)
   dp_back_kpa  = -huge(1.0_wp)
   ...
   do i = 1, probes%nprobes
      ...
      case ('front'); dp_front_kpa = dp_peak_kpa
      case ('back');  dp_back_kpa  = dp_peak_kpa
      end select
   end do

   fails = 0
   if (dp_front_kpa < FRONT_MIN_DP_KPA) then
      if (ctx%is_root) write(error_unit,'(A,F8.2,A,F8.2,A)') &
         'FAIL: front-face Δp_peak = ', dp_front_kpa, &
         ' kPa < threshold ', FRONT_MIN_DP_KPA, ' kPa'
      fails = fails + 1
   end if
   if (dp_back_kpa >= dp_front_kpa) then
      if (ctx%is_root) write(error_unit,'(A,F8.2,A,F8.2,A)') &
         'FAIL: back-face Δp_peak = ', dp_back_kpa, &
         ' kPa is not less than front = ', dp_front_kpa, ' kPa (cube not shadowing)'
      fails = fails + 1
   end if
   ...
   if (ctx%is_root) then
      if (fails == 0) then
         write(*,'(A)') 'test_blast_cube: PASS'
      else
         write(*,'(A,I0,A)') 'test_blast_cube: FAIL (', fails, ')'
      end if
   end if
   call mpi_finalize_ctx()
   if (fails /= 0) stop 1
end program test_blast_cube
